...
export default function UserDashboard() {
  ...
  const { data: statusData } = useQuery({
    ...
    queryFn: () =>
      user ? api.getUserStatus(user.email) : Promise.resolve(null),
    enabled: !!user,
  });
  ...
  const getStatusDisplay = (): { text: string; class: string } => {
    if (!statusData?.recording)
      return { text: "No submission", class: "bg-muted text-muted-foreground" };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-gradient-to-r from-primary/10 to-accent rounded-xl p-6 border border-border mb-6">
          <h2 className="text-2xl font-bold">
            Welcome to Your Audition Portal
          </h2>
          <p className="text-muted-foreground">
            Record, submit and track your audition.
          </p>
        </div>
          ...
          <div className="bg-warning/10 border border-warning/20 rounded-lg p-4 flex items-start space-x-3 mb-6">
            ...
            <div>
              <h3 className="font-semibold text-warning">
                Audition Portal Closed
              </h3>
              <p className="text-sm text-warning/80 mt-1">
                The audition submission period has ended. Check the leaderboard
                for final results.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            {!hasSubmitted && !isPortalClosed ? (
              ...
              <div className="bg-card rounded-xl p-8 text-center">
                ...
                <p className="text-muted-foreground">
                  {isPortalClosed
                    ? "The audition portal is closed."
                    : "Your audition is being processed."}
                </p>
                ...
                {recording && (
                  <div className="mt-6 space-y-4">
                    ...
                    <div>
                      ...
                      <span
                        className={`px-3 py-1 rounded-full ${statusDisplay.class}`}
                      >
                        {statusDisplay.text}
                      </span>
                    </div>
                    ...
                    {recording.ai_score !== null && (
                      <div className="mt-4 p-4 bg-accent rounded-lg">
                        ...
                        {/* Show full AI breakdown if available */}
                        {recording.ai_result &&
                          Array.isArray(recording.ai_result.results) && (
                              ...
                              <table className="w-full text-sm">
                                ...
                                <tbody>
                                  {recording.ai_result.results!.map(
                                    (r: AiFileScore, i: number) => (
                                      <tr key={i} className="border-t">
                                        <td className="px-3 py-2">
                                          {r.filename}
                                        </td>
                                        <td className="px-3 py-2 text-center">
                                          {(r.pitch * 100).toFixed(0)}%
                                        </td>
                                        <td className="px-3 py-2 text-center">
                                          {(r.rhythm * 100).toFixed(0)}%
                                        </td>
                                        <td className="px-3 py-2 text-center">
                                          {(r.tone * 100).toFixed(0)}%
                                        </td>
                                        <td className="px-3 py-2 text-center">
                                          {(r.expression * 100).toFixed(0)}%
                                        </td>
                                        <td className="px-3 py-2 text-center">
            ...
            <div className="bg-card rounded-xl p-6">
              ...
              {!statusData?.recording ? (
                ...
                <>
                  ...
                  <div className={`px-3 py-2 rounded ${statusDisplay.class}`}>
                    {statusDisplay.text}
                  </div>
                  {recording?.ai_score !== null && (
                    <>
